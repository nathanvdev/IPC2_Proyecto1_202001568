    ...
    while tmp1Next is not None:
        if tmp1.getColor() != tmp2.getColor() and tmp1.getColor() != tmp1Next.getColor() :
            if tmp1.getBlock() == False and tmp1.getPosY() == tmp1Next.getPosY() and tmp1Next.getBlock() == False:
                ...
                global MovCount
                MovCount += 1
                Instructions += '{}. Se Deslizo -->({}) de {},{} para {},{} -->({})\n'.format(MovCount,tmp1.getColor(),tmp1.getPosX(), tmp1.getPosY(),
                                                        tmp1Next.getPosX(), tmp1Next.getPosY(), tmp1Next.getColor())
                tmp = tmp1.getColor()
            ...
            while tmp1Down is not None:
                if tmp1Down.getPosX() == tmp1.getPosX() and tmp1Down.getPosY() == tmp1.getPosY()+1:
                    if tmp1.getBlock() == False and tmp1.getColor() != tmp1Down.getColor() and tmp1Down.getBlock() == False:
                        ...
                        MovCount += 1
                        Instructions += '{}. Se Deslizo -->({}) de {},{} para {},{} -->({})\n'.format(MovCount,tmp1.getColor(),tmp1.getPosX(), tmp1.getPosY(),
                                                        tmp1Down.getPosX(), tmp1Down.getPosY(), tmp1Down.getColor())
...
def InvertCell(L1: Cell_List, L2: Cell_List):
    ...
    while tmp1 is not None:
        if tmp1.getBlock() == False and tmp1.getColor() != tmp2.getColor():
            ...
            global MovCount
            MovCount += 1
            Instructions += '{}. se cambio de cara ({}) ({},{}) para obtener {}\n'.format(MovCount,tmp1.getColor(), 
                                                                                        tmp1.getPosX(), tmp1.getPosY(),
                                                                                        tmp2.getColor())
...
'''
                                            ...
                                            if ChosenFloor.getSlideCost() <= ChosenFloor.getFlipCost():
                                                SlidePatt(Cell_List1, Cell_List2)
                                                SlideDown(Cell_List1, Cell_List2)
                                                InvertCell(Cell_List1, Cell_List2)
                                            elif ChosenFloor.getSlideCost() > ChosenFloor.getFlipCost():
                                                InvertCell(Cell_List1, Cell_List2)
                                                SlidePatt(Cell_List1, Cell_List2)
                                                SlideDown(Cell_List1, Cell_List2)
